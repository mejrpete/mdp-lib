...
void THTS::disableCaching() {
    // Add a safety net of 5k nodes in case of early starved memory
    maxNumberOfNodes = std::max(lastUsedNodePoolIndex, 5000ul);
    maxDecisionNodes = std::max(numDecisionNodes, 5000ul);
...
void THTS::printStats(std::ostream& out, bool const& printRoundStats,
    ...
    if (currentTrial > 0) {
        ...
        out << indent << "Created SearchNodes: " << lastUsedNodePoolIndex
            << std::endl;
        out << indent << "Created Chance Nodes: " << numChanceNodes
            << std::endl;
        out << indent << "Created Decision Nodes: " << numDecisionNodes
            << std::endl;

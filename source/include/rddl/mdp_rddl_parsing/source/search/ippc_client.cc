...
IPPCClient::IPPCClient(std::string _hostName, unsigned short _port, double p)
    : hostName(_hostName),
      port(_port),
      socket(-1),
      numberOfRounds(-1),
...
void IPPCClient::initSession(string const& rddlProblem, string& plannerDesc) {
    ...
    planner->init(remainingTime);
    planner->initSession(numberOfRounds);
}
...
void IPPCClient::executeParser(string const& problemName,
    ...
    // Get home directory (required for Singularity)
    ...
    fs::path domainPath = homedir + "/parser_in.rddl";
    ...
    determinize(problemName, 600);

    Parser parser(homedir + "/" + problemName);
    parser.parseTask(stateVariableIndices, stateVariableValues);

    // Remove temporary files
    fs::remove(homedir + "/parser_in.rddl");
    fs::remove(homedir + "/" + problemName);

    // TODO(speckd): Write an alternative serach strategy if FD fails to compute
    // at least one layer
}

void IPPCClient::determinize(std::string const& /*problemName*/, int timeout) {
    // We use 10 min timelimit to generate the all-outcome determ.
    // and compute the DD-Heuristic
    struct passwd* pw = getpwuid(getuid());
    std::string homedir(pw->pw_dir);
    ...
    rddl_parser_cmd += homedir + "/parser_in.rddl ";
    rddl_parser_cmd += homedir + "/";

    Stopwatch det_timer;
    rddl_parser_cmd +=
        " -outcome_pruning " + std::to_string(pruning_probability);

    // CALL RDDL-PARSER
    std::cout << "\n\n\n"
              << std::string(15, '*') << " RDDL-PARSER CALL WITH "
              << std::to_string(timeout) << "SEC " << std::string(15, '*')
              << std::endl;
    int return_val = system(rddl_parser_cmd.c_str());
    timeout -= det_timer();
    if (return_val != 0) {
        std::cout << "RDDL-Parser timeout (or error)!" << std::endl;
    } else {
        std::cout << "RDDL-Parser took: " << det_timer << std::endl;
    }
}
